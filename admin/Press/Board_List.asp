...
%>
	<tr align=center>
		...
		<a href="Board_Read.asp?TableName=<%=TableName%>&idx_num=<%=Rs("idx_num")%>&Page=<%=Page%>&Search=<%=Search%>&Keyword=<%=Keyword%>"><%=Rs("Subject")%>
		</a>&nbsp;<%=new_data%></td>
		<td width=80 bgcolor=<%=bg%>><a href='mailto:<%=Rs("User_Email")%>' class='link'><%=Rs("User_Name")%></a></td>
