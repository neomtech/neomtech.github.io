...
dbCon.Open
'조회수 증가
Set Rs = dbCon.Execute("Update neo_notice Set Hits = hits+1 Where idx_num = "&Idx_Num&" ")


'리스트페이지에서 클릭한 제목과 같은글의 내용
Set Rs = dbCon.Execute("Select * From neo_notice Where  Idx_Num = "&Idx_Num&" ")
...
'이전글 쿼리
Sql="Select Max(idx_num) From neo_notice where idx_num < "&Idx_Num
...
'다음글 쿼리
Sql="Select Min(idx_num) From neo_notice where idx_num > "&Idx_Num
Set Rs=dbCon.Execute(Sql)
NextNum=Rs(0)
Rs.Close
Set Rs=Nothing
...
<div id="livereContainer">
	<script type="text/javascript">
		...
		var livere_seq 		= "10237";
		var smartlogin_seq 	= "228";

		var title = "네오엠텍 새소식 보러가기"
		var refer = "http://www.neomtech.com/news/news_view.asp?idx_num=<%=Idx_Num%>"
...
<br/>
						...
						<TABLE BORDER=0 CELLPADDING=0 CELLSPACING=0 WIDTH=600 class="doc" align=center>
							...
							<TR>
								<TD align=right>
								<!--<IMG SRC="images/icon_list.gif" WIDTH="61" HEIGHT="22" BORDER=0 ALT="">-->
								<%
								IF PrevNum > "" Then
								%>
									<a href="news_view.asp?idx_num=<%=PrevNum%>&nPage=<%=nPage%>"><img src="../img/but_prev.gif" width="55" height="19" border="0"> </a>
								<%
								Else
								%>
								...
								End IF
								%>
								<a href="news_list.asp?nPage=<%=nPage%>"><img src="../img/but_list.gif" width="55" height="19" border="0"></a> 
								<%
								IF NextNum > "" Then
								%>
									<a href="news_view.asp?idx_num=<%=NextNum%>&nPage=<%=nPage%>"><img src="../img/but_next.gif" width="55" height="19" border="0"></a>
								<%
								Else
								%>
