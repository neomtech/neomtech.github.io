...
If Ctext = "" Then '검색어가 없으면
	...
	sql="Select Count(idx_num) From neo_press"

Else '검색어가 있으면
	sql="Select Count(idx_num) From neo_press where "&Request("Cmenu")&" like '%"&Ctext&"%' "
End if
...
sql="Select Top "&Count&" * From neo_press Order by idx_num Desc"

Else'검색어가 있으면
	sql="Select Top "& Count &" * From neo_press where "&Request("Cmenu")&" like '%"&Ctext&"%' Order by idx_num Desc"
End if
...
<TABLE BORDER=0 CELLPADDING=0 CELLSPACING=0 WIDTH=100% class="doc">
	...
	<TR>
		...
		<TABLE BORDER=0 CELLPADDING=0 CELLSPACING=0 WIDTH=980 class="doc">
			<TR>
				<TD WIDTH=80 background="../images/back_s_left.gif" align=center valign=bottom><IMG SRC="../images/menu_left.gif" WIDTH="60" BORDER=0 usemap="#left_menu" ALT=""><br><br></TD>
				...
				<TABLE BORDER=0 CELLPADDING=0 CELLSPACING=0 WIDTH=900 class="doc">
					<TR>
						...
						<TABLE BORDER=0 CELLPADDING=0 CELLSPACING=0 WIDTH=600 class="doc" align=center>
							...
							'################긴제목 처리  #####################
							If len(Rs("Subject")) > 26 Then
								Title = Left(Rs("Subject"),24) + "..."
							Else
								Title=Rs("Subject")
							End if
							%> 
							<TR HEIGHT="28" align=center>
								...
								<TD><%=Formatdatetime(Rs("Reg_Date"),2)%></TD>
								<TD valign=bottom><IMG SRC="images/point.gif" WIDTH="1" HEIGHT="10" BORDER=0 ALT=""></TD>
								<TD style="padding-left:10;" align=left><a href='notice_view.asp?idx_num=<%=Rs("idx_num")%>&nPage=<%=nPage%>'><%=Title%></a></TD>
								<TD valign=bottom><IMG SRC="images/point.gif" WIDTH="1" HEIGHT="10" BORDER=0 ALT=""></TD>
								<TD><%=Rs("hit")%></TD>
							</TR>
							<%
					...
					IF blockpage <> 1 Then
					%>
						<a href="notice_list.asp?nPage=<%=blockpage-10%>"><img src="../img/ico_pre.gif" width="32" height="7" border="0"></a>
					<%
					Else
					%>
					...
					%> 
						...
						Else
						%>
							<a href="notice_list.asp?npage=<%=blockpage%>"><%=blockpage%></a> 
						<%
						End if
					...
					%>
						<a href="notice_list.asp?nPage=<%=blockpage%>"><img src="../img/ico_next.gif" width="32" height="7" border="0"></a>
					<%
					Else
					%>
